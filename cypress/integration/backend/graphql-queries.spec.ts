...
describe("A Query to the GraphQL Endpoint", () => {
	before(() => {
		cy.login();
	});

	it("validates the existance of reminders", () => {
		cy.task("seedDB", {
			reminders: [{
				_id: new ObjectId("611d515cc61d630f488dbfb4")
			}]
		});
		cy.graphQL(REMINDER_EXISTS, { id: "611d515cc61d630f488dbfb4" }).then(({data}) => {
			expect(data.reminderExists).to.be.true;
		});
		cy.graphQL(REMINDER_EXISTS, { id: "611d515cc61d630f488d0000"}).then(({data}) => {
			expect(data.reminderExists).to.be.false;
		})
	});

	it("validates the existance of categories", () => {
		cy.graphQL(CATEGORY_EXISTS, { id: "611d4fcfc61d630f488dbfab" }).then(({data}) => {
			expect(data.categoryExists).to.be.true;
		});
		cy.graphQL(CATEGORY_EXISTS, { id: "611d515cc61d630f488d0000"}).then(({data}) => {
