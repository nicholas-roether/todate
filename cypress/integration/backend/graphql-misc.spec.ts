...
function categoryDocToResponse(
	doc: seedDB.Doc,
	reminders?: seedDB.Doc
): { [key: string]: any } {
	...
	if (reminders)
		response.content = reminders.map((reminder) =>
			reminderDocToResponse(reminder)
		);
	return response;
}

function reminderDocToResponse(
	doc: seedDB.Doc,
	category?: seedDB.Doc
): { [key: string]: any } {
	const response: { [key: string]: any } = {
...
describe("The GraphQL Endpoint (concerning misc queries)", () => {
	...
	it("correctly gets current reminders", () => {
		 ...
		 */
		cy.task<seedDB.DocMap>("seedDB", "currentReminderTest").then(
			(docMap) => {
				const reminders = [
				...
				cy.graphQL(GET_CURRENT_REMINDERS).then(({ data }) => {
					...
					expect(
						currentReminders,
						"should return reminders correctly"
					).to.deep.equal(
						[
							reminders[0],
							reminders[1],
				...
				cy.graphQL(GET_CURRENT_REMINDERS, {
					...
					expect(
						...
						[reminders[6], reminders[7], reminders[8]].map(
							(reminder) => hashids.encodeHex(reminder._id)
						)
					);
				});
			}
	...
	it("correctly gets the reminder hierarchy", () => {
		 ...
		 */
		cy.task<seedDB.DocMap>("seedDB", "reminderHierarchyTest").then(
			(docMap) => {
				const reminders = [
					findInDocMap(docMap, "611eba8077f7e14488dad000"),
					findInDocMap(docMap, "611eba8077f7e14488dad001"),
				...
				const testCategory2 = findInDocMap(
					docMap,
					"611eba8077f7e14488dad100"
				);
				// Test 1
				cy.graphQL(GET_REMINDER_HIERARCHY).then(({ data }) => {
					const { categories, uncategorized } =
						data.getReminderHierarchy;
					expect(
						uncategorized,
						"should return uncategorized reminders correctly"
					).to.deep.equal(
						[
							reminders[0],
							reminders[1],
					...
					expect(
						categories,
						"should return categories correclty"
					).to.deep.equal([
						...
						categoryDocToResponse(testCategory2, [reminders[6]])
					]);
				});
				// Test 2
				cy.graphQL(GET_REMINDER_HIERARCHY, {
					...
					const { categories, uncategorized } =
						data.getReminderHierarchy;
					expect(
						uncategorized.map((rem) => rem.id),
						"should return uncategorized reminders correctly when bounded from below"
					).to.have.members(
						[
							reminders[0],
							reminders[1],
					...
					expect(
						categories.map((cat) => ({
							id: cat.id,
							content: cat.content.map((rem) => rem.id)
						})),
						"should correctly return categories when bounded from below"
					).to.deep.equal([
						{
							id: hashids.encodeHex(testCategory._id),
							content: [reminders[3], reminders[8]].map((rem) =>
								hashids.encodeHex(rem._id)
							)
						},
						{
							id: hashids.encodeHex(testCategory2._id),
							content: [reminders[6]].map((rem) =>
								hashids.encodeHex(rem._id)
							)
						}
					]);
				...
				cy.graphQL(GET_REMINDER_HIERARCHY, {
					...
					expect(
						uncategorized.map((rem) => rem.id),
						"should return uncategorized reminders correctly when bounded from above"
					).to.have.members(
						[reminders[0], reminders[1], reminders[4]].map(
							(reminder) => hashids.encodeHex(reminder._id)
						)
					);
					expect(
						...
						"should correctly return categories when bounded from above"
					).to.deep.equal([
						{
							id: hashids.encodeHex(testCategory._id),
							content: [
				...
				cy.graphQL(GET_REMINDER_HIERARCHY, {
					...
					expect(
						uncategorized.map((rem) => rem.id),
						"should return uncategorized reminders correctly when bounded from both sides"
					).to.have.members(
						[reminders[0], reminders[1], reminders[4]].map(
							(reminder) => hashids.encodeHex(reminder._id)
						)
					...
					expect(
						...
						"should correctly return categories when bounded from both sides"
					).to.deep.equal([
						{
							id: hashids.encodeHex(testCategory._id),
							content: [reminders[8]].map((rem) =>
								hashids.encodeHex(rem._id)
							)
						},
						{
