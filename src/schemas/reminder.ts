...
ReminderSchema.method({
	async findCategory(): Promise<CategoryDocument | null> {
		...
		return db.Category.findById(collectionId)
...
export interface ReminderQueryHelpers {
	before(
		date?: Date
	): mongoose.QueryWithHelpers<any, ReminderDocument, ReminderQueryHelpers>;
	after(
		date?: Date
	): mongoose.QueryWithHelpers<any, ReminderDocument, ReminderQueryHelpers>;
}
...
ReminderSchema.query.after = function (date?: Date) {
	if (!date) return this;
	return this.where(
		combineOr(this, [
			{
...
export type ReminderModelType = mongoose.Model<
	Reminder,
	ReminderQueryHelpers,
	ReminderMethods
>;
